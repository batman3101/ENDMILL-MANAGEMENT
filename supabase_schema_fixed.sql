-- ==================================================
-- CNC 앤드밀 관리 시스템 - Supabase 데이터베이스 스키마 (수정된 버전)
-- ==================================================
...
-- 사용자 확장 정보 테이블 (auth.users와 연동하되 직접 참조 제거)
CREATE TABLE user_profiles (
  id UUID PRIMARY KEY, -- auth.users.id와 수동으로 연결
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_by UUID -- auth.users.id를 저장하되 FK 제약 조건 제거
);
...
CREATE TABLE inventory (
  ...
  last_updated TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_by UUID, -- auth.users.id를 저장하되 FK 제약 조건 제거
...
CREATE TABLE inventory_transactions (
  ...
  purpose TEXT,
  notes TEXT,
  processed_by UUID NOT NULL, -- auth.users.id를 저장하되 FK 제약 조건 제거
...
CREATE TABLE cam_sheets (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_by UUID NOT NULL, -- auth.users.id를 저장하되 FK 제약 조건 제거
...
CREATE TABLE tool_changes (
  ...
  changed_by UUID NOT NULL, -- auth.users.id를 저장하되 FK 제약 조건 제거
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE translation_history (
  ...
  changed_by UUID, -- auth.users.id를 저장하되 FK 제약 조건 제거
  changed_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  reason TEXT
);
...
CREATE TABLE notifications (
  ...
  data JSONB DEFAULT '{}',
  recipient_id UUID, -- auth.users.id를 저장하되 FK 제약 조건 제거
...
CREATE TABLE activity_logs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID, -- auth.users.id를 저장하되 FK 제약 조건 제거
...
-- 사용자가 활성화되어 있는지 확인 함수 (수정된 버전)
CREATE OR REPLACE FUNCTION user_is_active()
RETURNS BOOLEAN AS $$
DECLARE
  is_active BOOLEAN;
BEGIN
  SELECT up.is_active INTO is_active
  FROM user_profiles up
  WHERE up.id = auth.uid();
...
-- 사용자 역할 테이블 정책
CREATE POLICY "사용자 역할 조회" ON user_roles FOR SELECT USING (user_is_active());
CREATE POLICY "시스템 관리자만 역할 관리" ON user_roles FOR ALL USING (get_user_role() = 'system_admin');

-- 사용자 프로필 테이블 정책
CREATE POLICY "본인 프로필 조회" ON user_profiles FOR SELECT USING (id = auth.uid() OR get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "본인 프로필 수정" ON user_profiles FOR UPDATE USING (id = auth.uid() OR get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "관리자만 사용자 생성" ON user_profiles FOR INSERT WITH CHECK (get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "시스템 관리자만 사용자 삭제" ON user_profiles FOR DELETE USING (get_user_role() = 'system_admin');

-- 설비 테이블 정책
CREATE POLICY "활성 사용자 설비 조회" ON equipment FOR SELECT USING (user_is_active());
CREATE POLICY "관리자 설비 수정" ON equipment FOR UPDATE USING (get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "시스템 관리자 설비 생성삭제" ON equipment FOR ALL USING (get_user_role() = 'system_admin');

-- 앤드밀 관련 테이블 정책
CREATE POLICY "활성 사용자 앤드밀 조회" ON endmill_categories FOR SELECT USING (user_is_active());
CREATE POLICY "활성 사용자 앤드밀 타입 조회" ON endmill_types FOR SELECT USING (user_is_active());
CREATE POLICY "관리자 앤드밀 관리" ON endmill_types FOR ALL USING (get_user_role() IN ('admin', 'system_admin'));

-- 공급업체 테이블 정책
CREATE POLICY "활성 사용자 공급업체 조회" ON suppliers FOR SELECT USING (user_is_active());
CREATE POLICY "관리자 공급업체 관리" ON suppliers FOR ALL USING (get_user_role() IN ('admin', 'system_admin'));

-- 재고 테이블 정책
CREATE POLICY "활성 사용자 재고 조회" ON inventory FOR SELECT USING (user_is_active());
CREATE POLICY "사용자 재고 업데이트" ON inventory FOR UPDATE USING (user_is_active());
CREATE POLICY "관리자 재고 생성삭제" ON inventory FOR INSERT WITH CHECK (get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "관리자 재고 삭제" ON inventory FOR DELETE USING (get_user_role() IN ('admin', 'system_admin'));

-- 재고 거래 정책
CREATE POLICY "활성 사용자 재고거래 조회" ON inventory_transactions FOR SELECT USING (user_is_active());
CREATE POLICY "활성 사용자 재고거래 생성" ON inventory_transactions FOR INSERT WITH CHECK (user_is_active());

-- 공구 위치 정책
CREATE POLICY "활성 사용자 공구위치 조회" ON tool_positions FOR SELECT USING (user_is_active());
CREATE POLICY "사용자 공구위치 업데이트" ON tool_positions FOR UPDATE USING (user_is_active());
CREATE POLICY "관리자 공구위치 관리" ON tool_positions FOR ALL USING (get_user_role() IN ('admin', 'system_admin'));

-- CAM Sheet 정책
CREATE POLICY "활성 사용자 CAM시트 조회" ON cam_sheets FOR SELECT USING (user_is_active());
CREATE POLICY "사용자 CAM시트 생성수정" ON cam_sheets FOR INSERT WITH CHECK (user_is_active());
CREATE POLICY "본인 또는 관리자 CAM시트 수정" ON cam_sheets FOR UPDATE USING (created_by = auth.uid() OR get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "관리자 CAM시트 삭제" ON cam_sheets FOR DELETE USING (get_user_role() IN ('admin', 'system_admin'));

-- CAM Sheet 상세 정책
CREATE POLICY "활성 사용자 CAM시트 상세 조회" ON cam_sheet_endmills FOR SELECT USING (user_is_active());
CREATE POLICY "사용자 CAM시트 상세 관리" ON cam_sheet_endmills FOR ALL USING (user_is_active());

-- 교체 이력 정책
CREATE POLICY "활성 사용자 교체이력 조회" ON tool_changes FOR SELECT USING (user_is_active());
CREATE POLICY "활성 사용자 교체이력 생성" ON tool_changes FOR INSERT WITH CHECK (user_is_active());
CREATE POLICY "본인 또는 관리자 교체이력 수정" ON tool_changes FOR UPDATE USING (changed_by = auth.uid() OR get_user_role() IN ('admin', 'system_admin'));
CREATE POLICY "관리자 교체이력 삭제" ON tool_changes FOR DELETE USING (get_user_role() IN ('admin', 'system_admin'));

-- 시스템 설정 정책
CREATE POLICY "활성 사용자 설정 조회" ON system_settings FOR SELECT USING (user_is_active());
CREATE POLICY "시스템 관리자만 설정 관리" ON system_settings FOR ALL USING (get_user_role() = 'system_admin');

-- 번역 정책
CREATE POLICY "활성 사용자 번역 조회" ON translations FOR SELECT USING (user_is_active());
CREATE POLICY "관리자 번역 관리" ON translations FOR ALL USING (get_user_role() IN ('admin', 'system_admin'));
...
-- 활동 로그 정책
CREATE POLICY "관리자만 로그 조회" ON activity_logs FOR SELECT USING (get_user_role() IN ('admin', 'system_admin'));
